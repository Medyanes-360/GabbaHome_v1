...
import CollectionTypeListItem from "./collectionTypeListItem";
import { useEffect, useRef, useState } from "react";
import useGlobalStore from "@/zustand/globalStore";
import { filterData } from "@/functions/util/dataFilter";

export default function CollectionTypeList() {
  const data = useCollectionTypeStore((state) => state.collectionTypes);
              ...
              {filteredData.length > 0 &&
                filteredData
                  ...
                  .map((collection, index) => {
                    return (
                      <CollectionTypeListItem key={index} data={collection} />
                    );
                  })}
              {filteredData.length == 0 && (
                <tr className="text-3xl w-full  truncate text-center">
                  <td colSpan={5} className="w-full !py-5">
