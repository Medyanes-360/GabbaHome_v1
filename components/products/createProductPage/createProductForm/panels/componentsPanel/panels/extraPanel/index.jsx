...
import CreateProductFormExtraListItem from "./listItem";

export default function CreateProductFormComponentsPanelExtraPanel() {
        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {productExtras.map((extra, index) => {
              return (
                <CreateProductFormExtraListItem data={extra} key={index} />
              );
            })}
            {[...Array(10 - productExtras.length)].map((e, index) => {
              return (
                <CreateProductFormExtraListItem
                  data={{
                    tempId: index,
                    isStandard: false,
                    price: "",
