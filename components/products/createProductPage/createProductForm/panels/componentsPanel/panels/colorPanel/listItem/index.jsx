...
export default function CreateProductFormColorListItem({ data }) {
  const addProductColor = useCreateProductFormStore(
    (state) => state.addProductColor
  );
  const removeProductColor = useCreateProductFormStore(
    (state) => state.removeProductColor
  );
  const productColors = useCreateProductFormStore(
    (state) => state.productColors
  );
  const [standardChecked, setStandardChecked] = useState(
    productColors.find((elem) => elem.colorId == data.id)?.isStandard
  );
  const [plusChecked, setPlusChecked] = useState(
    productColors.find((elem) => elem.colorId == data.id)?.price > 0
  );
  const [minusChecked, setMinusChecked] = useState(
    productColors.find((elem) => elem.colorId == data.id)?.price < 0
  );
  const [priceInputValue, setPriceInputValue] = useState(
    productColors.find((elem) => elem.colorId == data.id)?.price
  );
  const [isAddedToList, setIsAddedToList] = useState(
    // eğer component listeye eklenmişse disabled yap:
    productColors.find((elem) => elem.colorId == data.id) != undefined
  );
  ...
  const handleAddProductColor = () => {
    ...
    if (!standardChecked && (priceInputValue == 0 || priceInputValue == "")) {
      ...
      addProductColor(colorToAdd);
    }
  };
  // component'ı store'dan sil:
  const handleRemoveProductColor = () => {
    removeProductColor(data.id); // component'ı state'ten sil
    //inputları boşalt:
    setPlusChecked(false);
    setStandardChecked(false);
    setMinusChecked(false);
    ...
    <tr className="bg-white border-b  dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
      ...
      >
        {!isAddedToList ? (
          ...
          <ButtonComponent
            onClick={handleRemoveProductColor}
            text="Çıkar"
            classname="bg-red-500 px-4 py-2 rounded-md text-white "
          />
        )}
      </th>
      <td className="border-r px-6 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
        <CheckboxComponent
      ...
      <td className="border-r px-6 py-2 flex gap-3 items-center">
        <div
          style={{ backgroundColor: data.colorCode }}
          className="w-16 hover:scale-150 transition-all duration-200 cursor-pointer h-16 rounded-xl bg-transparent "
        ></div>
        <span>{data.colorCode}</span>
      </td>
      <td className="border-r px-6 py-2">
        <LanguageButton
          onClick={() => {
