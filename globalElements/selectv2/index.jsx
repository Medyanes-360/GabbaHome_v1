...
export default function SelectComponentV2({
  data,
  value,
  setvalue,
  placeholdertext,
  ...
  useEffect(() => {
    const selectedItem = data.find((elem) => elem.selected == true);

    if (selectRef.current) {
      if (selectedItem) {
        selectRef.current.value = selectedItem.value;
      } else {
    ...
    <>
      <FormControl error={errortext} fullWidth>
        ...
        >
          {data.map((elem, index) => {
            return (
              <MenuItem disabled={elem.disabled} key={index} value={elem.value}>
                {elem.name}
              </MenuItem>
            );
          })}
